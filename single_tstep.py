...
#   Plot the snapshot pinball curve along with the load distributions.
    ...
    plt.subplots_adjust(left=0.12, bottom=0.13, right=0.87, top=0.94, \
                        wspace=.1, hspace=.25)
        
    tot_l_dist_plt = profit_dist_fig.add_subplot(211)
    tot_l_dist_plt.plot(
        quan_wholesale, 
    ...
    profit_real.set_ylabel('Retailer Profit (£)', fontsize=13, color='purple')
    profit_real.tick_params(axis='y', labelcolor='purple')
    profit_real.legend(loc='upper left', bbox_to_anchor=(0.41, 1.225), ncol=2)
